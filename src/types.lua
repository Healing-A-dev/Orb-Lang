    ...
    elseif type(allowedTypes[varType].required) ~= "table" and assignment ~= nil then
        ...
        if varType ~= "Number" then
            ...
            if not assignment:match(allowedTypes[varType].required) and not varChecks.Real then
                ...
                if assignmentSUB:find("%(%)") then
                    local asSub = assignmentSUB:match("%w+%(%)"):gsub("%(%)","")
                    functionReturnType = Variables[utils.varCheck(asSub).Class:titlize()][asSub].Return_Type
                end
                if functionReturnType ~= varType then
...
function types.checkValue(variable,value,line)
    local varType,varValueType = utils.varCheck(variable).Type:lower(), type(utils.varCheck(variable).Value)
    print(utils.varCheck(variable).Value)
    if varType == "array" then varType = "table" end
    if varType == "char" then varType = "string" end
    if varType ~= varValueType and varType ~= "bool" and varType ~= "any" then
        print("NOT A PROPER VALUE DUMMY", variable, varType) --CHANGE TMRW
        os.exit()
    end
    return true
end
