	...
	for _,token in pairs(tokens) do
		...
		if not token.Token:find("VAREQL") and not token.Token:find("KEYWORD_GLOBAL") then
			...
			elseif token.Token:find("CONCAT") then
				var_value[#var_value] = concat(var_value[#var_value], tokens[_+1].Value, var_value[#var_value]:match("%S+%("), tokens[_+1].Value:match("%S+%("))
				skip_token = true
			end
...
-- 	print(var_data.Value,var_data.Value:match("%S+%("),var_name[1])
	var_data.DataType = getDataType(var_data.Value,var_data.Value:match("%S+%("))

	-- Adding variable to the variables table
	if add then
		VARIABLES[var_data.Classification:upper()][var_data.Name] = {Value = parent_value or var_data.Value, Type = var_data.DataType}
	end
	return var_data
end
